...
class Command(BaseCommand):
    ...
    @staticmethod
    def _generatete_verification_code() -> str:
        return os.urandom(12).hex()


    def handle_unverified_user(self, msg: Message, tg_user):
        code: str = self._generatete_verification_code()
    ...
    def handle_goals_list(self, msg: Message, tg_user):
        resp_goals: list[str] = [
            f'#{goals.id} {goals.title}'
            for goals in Goal.objects.filter(user_id=tg_user.user_id).order_by('created')
        ]
        if resp_goals:
    ...
    def handle_save_selected_category(self, msg: Message, tg_user: TgUser):
        if msg.text.isdigit():
            ...
            if GoalCategory.objects.filter(
                board__participants__user_id=tg_user.chat_id,
